...
@section('title', 'Ejercicios Hombros - Workout 3D')

@section('content')
    <section class="musculos-page">
        <header class="musculos-header">
            <h1>Ejercicios para Hombros</h1>
            <p class="musculos-subtitle">Aprende cómo entrenar hombros con estos ejercicios.</p>
        </header>
        ...
        <div class="musculos-container">
            <div class="embed-3d">
                <model-viewer 
                id="modelo" 
                src="{{ asset('model/hombro/hombro.glb') }}"
            ...
            <div class="pagination">
                @if($pagina > 1)
                    <a class="page-link" href="{{ route('hombros', ['pagina' => $pagina - 1]) }}">&laquo;</a>
                @else
                    <span class="disabled">&laquo;</span>
                @endif

                @for($p = 1; $p <= $totalPaginas; $p++)
                    <a class="page-link {{ $p == $pagina ? 'active' : '' }}" href="{{ route('hombros', ['pagina' => $p]) }}">{{ $p }}</a>
                @endfor

                @if($pagina < $totalPaginas)
                    <a class="page-link" href="{{ route('hombros', ['pagina' => $pagina + 1]) }}">&raquo;</a>
                @else
