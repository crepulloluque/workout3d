...
@section('title', 'Ejercicios Tríceps - Workout 3D')

@section('content')
    <section class="musculos-page">
        <header class="musculos-header">
            <h1>Ejercicios para Tríceps</h1>
            <p class="musculos-subtitle">Aprende cómo entrenar tríceps con estos ejercicios.</p>
        </header>
        ...
        <div class="musculos-container">
            <div class="embed-3d">
                <iframe 
                    ...
                    src="https://sketchfab.com/models/1f23b4ee5a1f4703b15a939412837638/embed">
                </iframe>
            </div>
            ...
            <div class="pagination">
                @if($pagina > 1)
                    <a class="page-link" href="{{ route('triceps', ['pagina' => $pagina - 1]) }}">&laquo;</a>
                @else
                    <span class="disabled">&laquo;</span>
                @endif

                @for($p = 1; $p <= $totalPaginas; $p++)
                    <a class="page-link {{ $p == $pagina ? 'active' : '' }}" href="{{ route('triceps', ['pagina' => $p]) }}">{{ $p }}</a>
                @endfor

                @if($pagina < $totalPaginas)
                    <a class="page-link" href="{{ route('triceps', ['pagina' => $pagina + 1]) }}">&raquo;</a>
                @else
